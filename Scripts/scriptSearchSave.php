...
<form class="formfix" action="scriptSearch.php" method="POST">
    <div class="searchBGcolor">
      <input class="searchbarshift" type="text" name="search" size="60px" placeholder="Script Search">   
	  	</div> 
...
<!--Add Script Button Note that this button is just a link and must remain outside the form -->
<a href="../Scripts/script_input.php"><button class="mediumbtn addButtonplace">Add Script</button></a>
</div>
</div>
            ...
            if ($queryResult > 0) {//Output Results only if greater than zero rows in database - otherwise do nothing
                while ($row = mysqli_fetch_assoc($result)){ //Note: row is the array prefix number or row number //Note: To get CSS in the results window requires single quotes on the class declaration
                    ...
                    echo " 
                        ...
                        <form method='post' action='scriptDropSearch.php?item_num=".$row['item_num']."&item_name=".$row['item_name']."'>
                        ...
                        <!--============= Break in echo - If Database pull = dropdown value add selected to attributes, otherwise continue echo ===========-->
                          ...
                          <option class='missing'  ";		if($row['status'] == 4)	{echo "selected = 'selected'";} echo" value='4'>Missing</option>
                            ...
                            <br>
                          <a href='scriptEdit.php?item_num=".$row['item_num']."&item_name=".$row['item_name']."'>
                            ...
                            <br>
                          <a id='ChangeUrl' onclick='notify()' href='scriptDelete.php?item_num=".$row['item_num']."&item_name=".$row['item_name']."'>
