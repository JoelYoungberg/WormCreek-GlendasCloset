...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
 ...
 <!-- Creating AJAX call function for dropdowns in return list -->
 <script>
function saveDrop(str) {
  if (str=="") {
  ...
  xmlhttp.open("GET","drop_ajax.php?v="+str,true);
  xmlhttp.send();
}
</script>
<script>
function saveIndex(str) {
  if (str=="") {
    document.getElementById("txtHint").innerHTML="";
    return;
  } 
  ...
  xmlhttp.open("GET","drop_ajax.php?i="+str,true);
  xmlhttp.send();
}
</script>
...
<input class="ReturnProcessor2" type="text" name="returnClerk" placeholder="Return Processor Name" form="returnOrder">
         ...
         if(!empty($_SESSION["skuArray"])){
            ...
            for ($i = 0; $i < count($skuArray); $i++){//Looping and outputting sku array with dropdown here
                echo "
					...
					class='selectReturns' onchange='saveIndex(this.id); saveDrop(this.value);'>
          ...
          ?>
            ...
            <form class="discForm" action='discountReturns.php' method='POST'>
              ...
              <div class="PercentButtons">
                ...
                <button class="btnsm" type='submit' name='submit-clear'>Clear</button> <!--Clear Button-->
            </div>
            </div>
            </form>
        <!--/////////////////////End Discount Form///////////////////////-->
        ...
        Amount Donated:&nbsp;&nbsp;$
        <input class="AmountPaidInput" type='number' min="0.01" step="0.01" name='amtPaid' size='20px' <?php if(isset($return)){echo "value = ". $row['amtPaid'] ."";}?> placeholder='Amount Paid' form="compOrder">
        </div>
<!-- Payment status and Complete Order-->
<form id="returnOrder" class="compOrderForm" method="POST" action="returnOrder.php">

    <input class="mediumbtn" type="submit" name="returnOrder" id="returnOrder" value="Return Order">

</form>

<form class="compOrderForm" method="post" action="cancelReturn.php">
    <input class="mediumbtn" type="submit" name="cancelReturn" id="cancelReturn" value="Cancel Return">
</form>

</div>
</body>

<div id="txtHint"><b>Status info will be listed here.</b></div>

</html>

<?php print_r($_SESSION['statusArray']);
