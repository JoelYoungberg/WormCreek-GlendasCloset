...
if ($numResultRowsCost > 0){
  ...
  header("Location: costume_input.php");
  exit();
}
...
$query = "INSERT INTO costumes (item_num, item_name, picture, descr, gender, size, color, loc, status, price, status_word)" .
"VALUES ('$item_num', '$item_name', '$fileDestination', '$descr', '$gender', '$size', '$color', '$loc', '$status', '$price', '$status_word')";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    //something posted
    header("Location: CostumeSearchForm.php?upload success");
    if (isset($_POST['save'])) {
        // btnDelete
    } else {
